...
class Sbucket extends EventEmitter {
   ...
   * @param {String} key - The key for the file to read
   * @param {Integer} length - an integer specifying the number of
   *                           bytes to read.
   * @param {Integer} position - an integer specifying where to begin
  ...
  readFileRange(key, position, length, callback) {
    ...
    async.eachSeries(indexRange, (index, next) => {
      ...
      return callback(null,
          Buffer.concat(buffList, (totalByes > length) ? length : totalByes));
    });
