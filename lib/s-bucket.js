...
class Sbucket extends EventEmitter {
  ...
  readFileRange(key, position, length, callback) {
    ...
    let indexEnd = Math.floor((position + length)/chunkSize);
    let indexRange = [];
    ...
    async.eachSeries(indexRange, (index, next) => {
      ...
      self._db.get(itemKey, (err, result) => {
        ...
        if (index === indexStart) {
          ...
          buffList.push(Buffer(result.slice(realStartPos, result.length), 'binary'));
        } else {
          buffList.push(Buffer(result, 'binary'));
        }
      ...
      return callback(null, Buffer.concat(buffList, length));
    });
