...
class Sbucket extends EventEmitter {
  ...
  /**
   * Reads the specified range of file at the given key into a buffer
   * @param {String} key - The key for the file to read
   * @param {Integer} length - an integer specifying the number of bytes to read.
   * @param {Integer} position - an integer specifying where to begin reading from in the file.
   * @param {Sbucket~readFileRangeCallback} callback
   */
  readFileRange(key, position, length, callback) {
    const self = this;
    const chunkSize = self._chunkSize;

    // calculate index range
    let indexStart = Math.floor(position/chunkSize);
    let indexEnd = Math.floor((position + length-1)/chunkSize);
    let indexRange = [];
    ...
    let buffList = [];
    let totalByes = 0;
    async.eachSeries(indexRange, (index, next) => {
      let itemKey = utils.createItemKeyFromIndex(key, index);
      // console.log('chunk:', index, itemKey, indexRange);

      self._db.get(itemKey, (err, result) => {
        if (err) {
          return next(err);
        }
        ...
        let temp = null;
        if (index === indexStart) {
          // need to handle position is not aligned with chunkSize
          let realStartPos = position - index * chunkSize;
          temp = Buffer(result.slice(realStartPos, result.length), 'binary');
        } else {
          temp = Buffer(result, 'binary');
        }

        totalByes += temp.length;
        buffList.push(temp);
      ...
      if (err && !utils.isNotFoundError(err)) {
        return callback(err);
      }

      return callback(null, Buffer.concat(buffList, (totalByes > length) ? length : totalByes));
    });

    self._incPendingOps();
  }
  /**
   * @callback Sbucket~readFileRangeCallback
   * @param {Error} [error]
   * @param {Buffer} fileBuffer
   */
