...
/**
 * Coerces input into a valid file key or bucket index
 * @param {String} keyOrIndex - The bucket index or a file key
 * @returns {String|Number}
 */
module.exports.coerceKeyOrIndex = function(keyOrIndex) {
  var key = keyOrIndex;

  if (!module.exports.isValidKey(keyOrIndex)) {
    key = parseInt(keyOrIndex);

    if (Number.isNaN(key)) {
      key = module.exports.hashKey(keyOrIndex);
    }
  }
