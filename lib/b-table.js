...
class Btable extends EventEmitter {
  ...
  _open() {
    ...
    for (let i = 0; i < constants.B; i++) {
      this._getSbucketAtIndex(i).open(utils.noop);
    }
  }
   ...
   */
  flush(callback) {
    async.eachOfSeries(this._sBuckets, (sBucket, i, done) => {
      async.waterfall([
        (next) => sBucket.flush(next),
        (next) => this._getSbucketAtIndex(parseInt(i), next),
        (sBucket, i, next) => sBucket.list(next)
      ], done);
    }, callback);
  }
  /**
